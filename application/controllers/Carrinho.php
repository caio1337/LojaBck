...
class Carrinho extends CI_Controller{
    ...
    public function insert(){
        ...
        if(!$produto_id || $produto_quantidade < 1){
            ...
            $retorno['erro'] = 3;
            $retorno['mensagem'] = 'Informe quantidade maior que 0';

        }else{
            //Continua o processamento, verificar existencia do produto
            ...
            $retorno['mensagem'] = 'Produto não encontrado.';
                ...
                //Sucesso.. Produto existe, comparar quantidade em estoque com o requisitado.
                
                if($produto_quantidade > $produto->produto_quantidade_estoque){

                    $retorno['erro'] = 3;
                    $retorno['mensagem'] = 'Ops, só temos '.$produto->produto_quantidade_estoque.' unidade em estoque.';
        ...
        if($produto_quantidade == "" || $produto_quantidade < 1){
            ...
            $retorno['mensagem'] = 'Produto não encontrado.';
                ...
                if($produto_quantidade > $produto->produto_quantidade_estoque){
                    ...
                    $retorno['erro'] = 3;
                    $retorno['mensagem'] = 'Ops, só temos '.$produto->produto_quantidade_estoque.' unidade(s) em estoque';
                }else{
                    //Sucesso, atualiza a quantidade do intem, pois há estoque...
    ...
    public function calcula_frete(){
        ...
        if($this->form_validation->run()){
            ...
            if (isset($resultado->erro)) {
                ...
                $retorno['retorno_endereco'] = '<br><br>'. 'Não encontramos o CEP em nossa base de dados';
            } else {
                $retorno['erro'] = 0;
...
//                    $frete_calculado .= '<p>' . ($dados->Codigo == '04510' ? 'PAC' : 'Sedex') . '&nbsp;R$&nbsp;' . $valor_calculado . ', <span class="badge badge-primary py-0 pt-1">' . $dados->PrazoEntrega . '</span> dias úteis<p>';
            ...
            $retorno['erro'] = 0;
            $retorno['retorno_endereco'] = '<br><br>'. $retorno_endereco . '<br>' . $frete_calculado;
            }

        }else{

            $retorno['erro'] = 3;
            $retorno['retorno_endereco'] = '<br><br>'. validation_errors();
        }
