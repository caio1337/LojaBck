    ...
    public function index() {
        ...
        if($this->form_validation->run()){
            ...
            if(!$produto = $this->core_model->get_by_id('produtos', array('produto_id' => $produto_id))){
                ...
                //Inicio da consulta Web Service viacep
            ...
            //Montando url para consultar cep
            ...
            logradouro: "Rua Emanuel Marzano Matias"
            siafi: "4123"
            uf: "MG"
            */
            ...
            //Final da consulta via cep

            //Inicio da consulta dos correios

            //Montando url para os correios exibir o valor do frete
            ...
            //&nCdFormato=1&nVlComprimento=20&nVlAltura=20&nVlLargura=20&sCdMaoPropria=n&nVlValorDeclarado=0&sCdAvisoRecebimento=n&nCdServico=04510&nVlDiametro=0&StrRetorno=xml&nIndicaCalculo=3
           ...
           //Transformando o doc xml em objeto
           $xml = simplexml_load_file($url_correios);
           $xml = json_encode($xml);

           $consulta = json_decode($xml);

           //Garantindo que houve sucesso na consulta ao webservice dos correios
           if($consulta->cServico[0]->Valor == '0,00'){
           
            $retorno['erro'] = 3;
            $retorno['retorno_endereco'] = 'Não foi possivel calcular o frete, entre em contato com nosso suporte.';
            exit();
               ...
               foreach($consulta->cServico as $dados ){
                ...
                $frete_calculado .= '<p>'.($dados->Codigo == '04510' ? 'PAC' : 'Sedex').'&nbsp;R$&nbsp;'.$valor_calculado.', <span class="badge badge-primary">'.$dados->PrazoEntrega.'</span> dias. </p>';
