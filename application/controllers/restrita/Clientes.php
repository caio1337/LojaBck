    ...
    public function core($cliente_id = null) {
        ...
        if (!$cliente_id) {
            ...
            if ($this->form_validation->run()) {
                ...
                $group = array('2'); //Seta o usuario para o grupo de clientes
...
//                    exit();
    ...
    public function delete($cliente_id = NULL) {
        ...
        if (!$this->ion_auth->is_admin()) {
            $this->session->set_flashdata('erro', 'Você não tem permissão para acessar esse menu');
            redirect('restrita/clientes');
        }
        ...
        if ($this->core_model->get_by_id('users', array('cliente_user_id' => $cliente_id, 'active' => 1))) {
            $this->session->set_flashdata('erro', 'Não é possível excluir um cliente com usuario ativo');
            redirect('restrita/clientes');
        }
